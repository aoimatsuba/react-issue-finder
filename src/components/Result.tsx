...
const Result: React.FunctionComponent = () => {
    ...
    const statusFilter = useSelector((state: RootState) => state.issueStatus)
    ...
    if (data?.search?.edges === undefined || data?.search?.edges.length < 1) {
        return <div className="text-center m-5">No results matched your search.</div>
    } else {
        return (
            <div className="results__container">
                {data?.search?.edges?.map((edge) => (
                    <ResultItem key="" issue={edge.node} isDetail={false} />
                ))}
            </div>
        )
    }
